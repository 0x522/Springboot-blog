...
public class AuthController {
    ...
    public Object auth() {
        ...
        if (loggedInUser == null) {
            ...
            return new Result("ok", "用户已经登录", true, loggedInUser);
        }
    }
    ...
    public Result register(@RequestBody Map<String, String> usernameAndPassword) {
        ...
        if (username == null || password == null) {
            return Result.failure("用户名或者密码是空");
        }
        if (!username.matches("^\\w{1,15}$")) {
            return Result.failure("用户名不符合规则，请输入1-15位字符数字下划线汉字");
        }
        if (!password.matches("^.{6,16}$")) {
            return Result.failure("密码不符合规则，请输入6-16位任意字符");
        }
        try {
